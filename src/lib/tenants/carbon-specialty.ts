...
export const carbonSpecialtyConfig: TenantIntakeConfig = {
  ...
  output: {
    ...
    // {{camelCase}} placeholders map 1:1 to CarbonIntakePayload fields
    // (habitational COPE schema). The /api/lead-fallback route fills
    // these and sends the result as the plain-text email body.
    summaryTemplate: [
      ...
      "  Central alarm:   {{centralStationAlarm}}",
      "  Electrical:      {{electricalType}}",
      "",
      "COVERAGE & FINANCIALS",
      "  Gross annual rents:   {{grossAnnualRents}}",
      "  Effective date:       {{effectiveDate}}",
      "  Current carrier:      {{currentCarrier}}",
      "  Expiring premium:     {{expiringPremium}}",
      "  Markets consent:      {{consent}}",
      "  Enrichment confirmed: {{enrichmentConfirmed}}",
      "",
      "LOSS HISTORY (self-reported, 5yr)",
      "{{lossHistory}}",
      "",
      "FLAGS",
      "  Flood concern:   {{floodConcern}}",
