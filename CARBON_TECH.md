...
- `src/lib/ca-county-registry.ts` (C.S.1.7.0a, expanded C.S.1.7.0b) — per-county `CACountyConfig` registry. Pre-populated with **LA, San Diego, Orange, Riverside** counties. SB intentionally skipped — SB's public ArcGIS surfaces only geometry + APN (no building/owner/transaction attributes); see CARBON_RESEARCH.md "Property data API landscape" for the SB coverage gap. Owner fields are explicitly modeled but mostly `undefined` for CA per the broader privacy gap; **Orange + Riverside DO publish owner mailing address** through their public ArcGIS — first two CA counties in the registry to do so.
