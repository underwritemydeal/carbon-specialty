...
- `src/lib/socrata-client.ts` (C.S.1.7.0d) — generic Socrata SoQL query client. Geometry queries via `within_circle(geometryField, lat, lon, meters)`, plus where + baseWhere + select + limit. Exports `querySocrataDataset` + `buildSocrataUrl` + `SOCRATA_DEFAULT_RADIUS_M`. Silent-null on every failure mode with `[carbon-enrich] SOCRATA_*` diagnostic logging. No auth required for DataSF.
- `src/lib/construction-sanity.ts` (C.S.1.7.0e) — IBC-physics sanity check that suppresses obviously-wrong construction codes for tall buildings. Three rules: 7+ story wood frame, 10+ story heavy timber, 12+ story not-Type-I/not-Steel (only when a recognizable lower-type marker is present — raw codes like LA's `0500` pass through). Sets `building.constructionTypeFlag = "unreliable_county_data"` when a rule fires; chat asks the user for actual construction instead of surfacing the bad string. Applied at the end of `fetchCACounty` (both ArcGIS + Socrata paths). C.S.1.7.0f added LA construction mapping (QualityClass1 leading letter → IBC type) so sanity check fires on LA records too.
- `src/lib/ca-county-registry.ts` (C.S.1.7.0a, expanded + insurance-tuned in C.S.1.7.0b, table-join in C.S.1.7.0c, Socrata client in C.S.1.7.0d) — per-county `CACountyConfig` registry. Pre-populated with **LA, San Diego, Orange, Riverside, San Francisco** counties. SB intentionally skipped (no public attribute layer — see CARBON_RESEARCH.md). Field scope follows insurance-underwriting priorities: CRITICAL (use code, year built, building sqft, units, construction, stories) + USEFUL (effective year built, sprinklered, roof type, bedrooms, bathrooms). DROP-list fields (lot sqft, sale data, assessed value, tax-exempt indicators) were removed from the type system AND the registry entries in C.S.1.7.0b — those are appraisal/marketing concerns, not underwriting ones. **Orange + Riverside publish owner mailing address** (rare for CA) — staged for the marketing-export sprint but not surfaced to Carbon's chat. **C.S.1.7.0c added the optional `assessorTableJoinUrl` + `assessorTableJoinKey` + `assessorTableJoinForeignKey` + `tableJoinFields` config** to support counties that split parcel geometry from building characteristics across two ArcGIS layers (recurring CA shape). Riverside is the first user: PARCELS_CREST (MapServer/50) joins to CREST_PROPERTY_CHAR (MapServer/80) via APN→PIN, lighting up year_built, building_sqft, stories, construction_type, bedrooms, bathrooms, roof_type that the primary layer doesn't publish. Units is intentionally not mapped in the join — CREST_PROPERTY_CHAR doesn't publish a unit-count field; Riverside encodes apartment unit ranges in CLASS_CODE strings on the primary layer ("Apartment 21 - 40 Units").
- `src/lib/fetch-ca-county.ts` (C.S.1.7.0a, table-join in C.S.1.7.0c, Socrata dispatch in C.S.1.7.0d) — wraps `arcgis-client` + `socrata-client` + `ca-county-registry`. Exports `fetchCACounty(lat, lon, detectedCounty)`, the `normalizeCountyFeature` field-mapping translator, and (new in C.S.1.7.0c) `fetchAssessorTableJoin` + `aggregateJoinRows` + `mergeJoinedBuilding`. **C.S.1.7.0d added client-type dispatch**: `county.client === "arcgis"` takes the FeatureServer path (LA, SD, OC, Riverside); `county.client === "socrata"` takes the SoQL path (SF). When a county config carries the join trio, the fetcher fires a second where-clause query against the joined table after the primary success, aggregates multi-row responses insurance-tuned (year=MIN, sqft=SUM, stories=MAX, construction_type/roof=MODE, bedrooms/bathrooms=SUM), and merges into PropertyFacts.building (primary wins on conflict). 100-row cap per parcel (live-probed at up to 22 rows for a 100+ unit apartment complex). Silent-null on join failure — primary data still ships (graceful degradation). Used by `/api/property/enrich` ahead of the Realie fallback for CA addresses.
